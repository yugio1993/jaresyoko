...
// データ管理
// ==============================
let books = []; // 本のデータは localStorage に保存されますが、内容は 'books/' フォルダ内にテキストファイルとしても保存されます。
...
function calculatePagination(jumpToPage = 0) {
    ...
    // 縦書き(vertical-rl)において、最初のページを右側に、次のページを左側に配置する。
    // translate(0) の時に一番右側（開始位置）が見えるようにする。
...
function updatePaginationUI() {
    const vw = bookViewport.clientWidth;
    const pageAdvance = vw + 100;
    // 縦書き(vertical-rl)ではスクロール方向が負（右から左）になるため、
    // translateX(-currentPage * pageAdvance) で左側のページへ進む
    ...
    Array.from(files).forEach(file => {
        ...
        reader.onload = (event) => {
            ...
            if (processed === files.length) {
                saveBooks();
                renderBookshelf();
                showToast(`${files.length}冊の本を書庫に追加しました。`);
            }
        };
...
function setupEventListeners() {
    ...
    bulkDeleteBtn.addEventListener('click', () => {
        ...
        books.forEach(b => {
            const item = document.createElement('div');
            item.className = 'download-book-item';
            item.innerHTML = `<input type="checkbox" id="del-${b.id}" data-id="${b.id}">
                             <label for="del-${b.id}">${b.title}</label>`;
            deleteBookList.appendChild(item);
        });
    ...
    selectAllDeleteBtn.addEventListener('click', () => {
        deleteBookList.querySelectorAll('input').forEach(cb => cb.checked = true);
    });

    deselectAllDeleteBtn.addEventListener('click', () => {
        deleteBookList.querySelectorAll('input').forEach(cb => cb.checked = false);
    });

    doDeleteBtn.addEventListener('click', () => {
        const selected = [...deleteBookList.querySelectorAll('input:checked')];
        if (selected.length === 0) return;
        if (confirm(`${selected.length}冊の本を削除しますか？`)) {
