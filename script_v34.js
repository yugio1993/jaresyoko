...
async function loadBooks() {
    ...
    if (saved) {
        ...
        // サーバー上の initial_data.json を確認
        const initialData = await fetchInitialData();
        if (initialData) {
            bookshelves = initialData.bookshelves || [];
            books = initialData.books || [];
            currentBookshelfId = initialData.currentBookshelfId || (bookshelves[0] ? bookshelves[0].id : null);
            showToast('初期設定データを読み込みました。');
        } else {
            // V27以前のデータからの移行
...
async function fetchInitialData() {
    try {
        ...
        console.log('No initial_data.json found.');
    }
    return null;
}
...
function setupEventListeners() {
    ...
    backupBtn.addEventListener('click', exportLibrary);
...
function exportLibrary() {
    const data = { bookshelves, books, currentBookshelfId, version: "v34" };
...
function importLibrary(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (data.bookshelves && data.books) {
                if (confirm('現在の書庫データが上書きされます。よろしいですか？')) {
                    bookshelves = data.bookshelves;
                    books = data.books;
                    currentBookshelfId = data.currentBookshelfId || (bookshelves[0] ? bookshelves[0].id : null);
                    saveBooks();
                    renderBookshelf();
                    showToast('書庫データを復元しました。');
                }
            } else {
                alert('無効なデータ形式です。');
            }
