...
const ssSyncBtn             = document.getElementById('ss-sync-btn');
...
async function init() {
    ...
    const savedUrl = localStorage.getItem('bookshelf_ss_url');
    if (savedUrl) ssUrlInput.value = savedUrl;
...
function setupEventListeners() {
    ...
    ssSyncBtn.addEventListener('click', () => ssModal.classList.remove('hidden'));
    closeSsModal.addEventListener('click', () => ssModal.classList.add('hidden'));
    doSsSyncBtn.addEventListener('click', async () => {
        const url = ssUrlInput.value.trim();
        if (!url) { alert('URLを入力してください。'); return; }
        
        localStorage.setItem('bookshelf_ss_url', url);
        await syncWithSpreadsheet(url);
    });

    window.addEventListener('click', () => contextMenu.classList.add('hidden'));
}

async function syncWithSpreadsheet(url) {
    try {
        showToast('スプレッドシートからデータを取得中...', 'forest');
        ...
        if (data.length === 0) {
            alert('データが見つかりませんでした。CSV形式が正しいか確認してください。');
            return;
        }
        ...
        data.forEach((row, idx) => {
            const shelfName = row.shelfName || 'スプレッドシートの本棚';
            if (!shelfMap.has(shelfName)) {
                const sId = Date.now() + Math.random();
                shelfMap.set(shelfName, sId);
                newBookshelves.push({ id: sId, name: shelfName });
            }
            ...
            newBooks.push({
                ...
                pattern: row.pattern || 'antique',
                lastPage: 0,
                date: Date.now() - idx,
                shelfId: shelfMap.get(shelfName)
            });
        });

        if (confirm(`${newBooks.length} 冊の物語を読み込みます。現在のデータは上書きされます。よろしいですか？`)) {
        ...
        console.error('SS Sync Error:', error);
        alert('同期に失敗しました。URLが正しいか、Googleスプレッドシートが「ウェブに公開(CSV)」されているか確認してください。');
    }
}
