...
function setupEventListeners() {
    ...
    addBookForm.addEventListener('submit', (e) => {
        ...
        if (id) {
            ...
            if (b) {
                db.collection('books').doc(b.firestoreId).update({ title, content, color, pattern });
            }
        } else {
            // 新規追加
            const newBook = { id: Date.now(), title, content, color, pattern, date: Date.now(), shelfId: currentBookshelfId };
